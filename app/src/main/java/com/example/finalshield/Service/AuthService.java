...
public class AuthService {
    ...
    public String obtenerPinReal() { return prefs.getString(PIN_REAL_KEY, ""); }
    public String obtenerPinSeguro() { return prefs.getString(PIN_SEGURO_KEY, ""); }
    private void guardarPinReal(String pin) { if(pin != null) prefs.edit().putString(PIN_REAL_KEY, pin).apply(); }
    private void guardarPinSeguro(String pin) { if(pin != null) prefs.edit().putString(PIN_SEGURO_KEY, pin).apply(); }

    // --- MÉTODOS DE PERSISTENCIA (TOKEN, CORREO, NOMBRE) ---
    public void guardarToken(String token) { prefs.edit().putString(TOKEN_KEY, token).commit(); }
    public String obtenerToken() { return prefs.getString(TOKEN_KEY, null); }
    public void guardarCorreo(String correo) { prefs.edit().putString(CORREO_KEY, correo).commit(); }
    public String obtenerCorreo() { return prefs.getString(CORREO_KEY, null); }
    public void guardarNombre(String nombre) { prefs.edit().putString(NOMBRE_KEY, nombre).apply(); }
    public String obtenerNombre() { return prefs.getString(NOMBRE_KEY, "Usuario"); }

    // --- GESTIÓN DE SESIÓN (RESTAURADO) ---
    public boolean isLoggedIn() { return obtenerToken() != null; }
    ...
    public void habilitarBiometrico(String correo, boolean activado, Callback<Void> callback) {
        ...
        api.habilitarBiometrico(body).enqueue(new Callback<ResponseBody>() {
            ...
            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) { callback.onFailure(null, t); }
        });
    }
